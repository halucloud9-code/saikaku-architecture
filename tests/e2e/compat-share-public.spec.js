...
test('public v1 share fetches and renders without visual-only matched-term guidance', async ({ page }) => {
  ...
  await expect(page.locator('.compat-availability-row strong', { hasText: '旧A氏' })).toBeVisible();
  await expect(page.locator('.compat-availability-row strong', { hasText: '旧B氏' })).toBeVisible();
  await expect(page.getByText(/人事評価・採用評価には流用しません/)).toBeVisible();
});
