...
test('attemptCount=3 blocks starting another saikaku diagnosis', async ({ page }) => {
  ...
  await expect(page.getByTestId('badge-saikaku')).toContainText('診断済み (3/3)', { timeout: 15000 });
  await expect(page.getByText('診断は最大3回まで実施済み')).toBeVisible();
  ...
  expect(alertMessages[0]).toContain('診断は最大3回まで実施済みです');
