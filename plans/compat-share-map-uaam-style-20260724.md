...
## 2026-07-24 追加オーナー決定（つかさ）: セル判定をチーム平均基準へ

- 「プロとナチュラルの基準低くない？」→ 最良ゾーンの重ね合わせ（union）は4人チームで明るくなりすぎる。**セル色 = チーム平均の判定**に変更（つかさ明示指示「平均を取るか基準を人数倍してよ」）
- 実装は整数比較（軸ごと: sum_i vs threshold×count_i。ペア和条件はクロス乗算 sum_i×count_j + sum_j×count_i vs threshold×count_i×count_j）で小数誤差を排除。NATURAL は全員20×20のみ
- 軸ごとにデータ保有者だけで平均（0点埋め禁止は維持）。ホバーの個人別判定・担い手表示は不変。TOP5/zAlpha/sharedMatrix/topPairs は平均基準に追随
- 受講者さがしの不足軸基準（在庫: 軸最高値<12）は**変更しない**（役割分離: 地図=水準、さがし=穴埋め）。R2 debate の「別人合成禁止」は union の文脈であり、平均への変更はオーナー決定として design-rationale に記録
