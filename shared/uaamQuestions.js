...
export function computeUaamQuestionVersion(questions = QUESTIONS) {
  const versionSource = JSON.stringify(questions.map(({ id, text, reverse, axis, sub }) => (
    [id, text, reverse, axis, sub]
  )));
  let first = 0x811c9dc5;
  let second = 0x9e3779b9;
  for (let index = 0; index < versionSource.length; index += 1) {
    const code = versionSource.charCodeAt(index);
    first = Math.imul(first ^ code, 0x01000193);
    second = Math.imul(second ^ code, 0x85ebca6b);
  }
  return `uaam64-${(first >>> 0).toString(16).padStart(8, '0')}${(second >>> 0).toString(16).padStart(8, '0')}`;
}

export const UAAM_QUESTION_VERSION = computeUaamQuestionVersion();
