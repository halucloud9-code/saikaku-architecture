...
- **受講者さがしは順位付けしない二段階開示**: 不足軸（チーム最高値<12 または データなし）を16以上で持つ内部受講者のみ対象。第1段階は軸ごとの人数集計だけ（氏名なし）、第2段階は個別同意確認チェック後に氏名＋該当軸チップを表示名照合順で表示。有用度ソート・件数バッジ・評価コピーは禁止。当初の「弱セル数降順Top5」は実質ランキングとして棄却。public メンバー混在は 400（架空URL による全受講者列挙の遮断）。profileId はレスポンスに含めない。実行は両段階とも compat_audits に記録。
