...
**2026-08-01 owner decision（方針転換）**: 上の禁止は「**人物どうしの相性を単一の点数にすること**」に限定して維持し、「学び合いやすい組み合わせ」を示す決定論的な指標については解禁した。経緯と、当時の懸念2点をどう解いたかは次項に記す。

### 学び合い指標を解禁した理由（2026-08-01）

2026-07-24 の受講者さがし（`plans/compat-matrix-names-20260724.md`）では「相性の数値スコア・順位付け・有用度ソート・評価コピー」をスコープ外と決め、候補を表示名順に並べるだけにした。当初案の「弱セル数降順Top5」も実質ランキングとして棄却している。

2026-08-01 につかさが方針転換を承認した。用途が「コミュニティ内でチームを組むための相手さがし」と確定し、受講者が内部コミュニティで同意済みであることが示されたため。雇用関係の人事評価・合否判定・配属判定ではないので、`evaluation-ethics` の禁止対象には当たらない。

当時の懸念2点への解:

- **説明可能性が落ちる** → 指標を単一のブラックボックス値にせず、内訳（平均点の差／120マス中の食い違いマス数）を必ず同じ画面に併記する。指標は2つの観測値だけから決まる。
- **人事評価・採用評価への流用を誘発する** → 用途表記を「コミュニティ内のチームづくりと相互理解のためのもの。雇用・人事・採用・査定の判断には使いません」に改め、指標は個別同意確認後の第2段階でのみ返し、共有ページ・印刷・エクスポートには一切載せず、`compat_audits` に `algorithmVersion` / `rankingShown` / `displayedCandidateCount` を記録する。LLM 出力側のスコア禁止契約（`validateCompatOutput.js`）は無変更で維持する。

### 学び合い指標の式と、なぜゾーン階級差を使わないか

指標 = `round(100 × √(L × D))`（0〜100の整数）。

- `L`（水準の近さ）= `max(0, 1 − 16軸平均点の差 / 4.0)`。平均点の差が4点以上なら0＝候補から外れる。
- `D`（地図の違い）= 120マスのうち「発動している（natural/pro/active）／していない」が対象と候補で食い違うマスの割合を rate として `min(1, rate / 0.6)`。

積なのでどちらかが0なら0になり、「水準が近い**かつ**地図が遠い」という AND を満たす。平方根は順位を変えない単調変換で、表示値の分布を読みやすくするためだけに使う。

**ゾーンの階級差（`ZONE_RANK` の差の平均）を距離に使わない理由**: 本番51名の実データで測ると、ゾーン階級差は16軸平均点の差と **r = 0.89** で相関する。つまりそれは「水準の差」の言い換えであり、`L` と掛け合わせると2条件が打ち消し合って全ペアが0〜26点（中央10点）に潰れる。「片方だけ発動しているマスの割合」は同じ相関が **r = 0.23** にとどまり、実データで中央55点・最大98点の分布が出る。

**採用しなかった候補資格ゲート**: sol は「不足軸を16点以上で持つ人だけを候補にする」ゲートの併用を提案したが、実データで **51人中17人（33%）が候補ゼロ**になる（水準の高い人ほど不足軸を持たないため）。必須ゲートにはせず、該当する場合の説明チップとしてのみ表示する。

**決定論**: 二段階開示のスナップショットは推薦と指標を一体で sha256 する。返却値に浮動小数を入れない（チーム軸平均は 2520 = 1..10 の最小公倍数を固定スケールにした整数比で保持し、丸めは整数比較で解く）。並びは 指標降順 → 食い違いマス数降順 → 平均点差昇順 → `profileId` のコードポイント昇順。**`localeCompare` は順位判定に使わない**（ICU 版差で順序が変わりスナップショット契約が壊れるため）。
...
Anthropicへ送る識別子は M1…だけとする。氏名、メール、UID、自由記述は送らない。本人が入力したTop5の語はLLMに送信されない。生成軸名は別名化プロフィールの一部としてLLMに渡る。完全一致について送るのは件数と分類だけである。ほかにUAAM percentile由来の証拠を送る。入力データは命令ではないことをsystem promptで明示し、出力後は未知の証拠ID、claim契約違反、相性スコア、人事・採用・査定語を決定論的に拒否する。修復再試行は1回だけで、再失敗時はレポートを返さない。旧 A/B 形式の共有結果は読取互換を維持するが、表示名への置換は行わない。

互換分析のモデルは `COMPAT_MODEL` で上書きでき、既定値を `claude-sonnet-4-6` に明示固定する。リポジトリ内の他診断のモデル設定には追従させない。

### 相性マンダラと深掘りv2（2026-07-15 owner decision）

テキストだけでは対象者が証拠の全体像を把握しにくかったため、レポート先頭を `§0 データの範囲 → 相性マンダラ → 同質・補完の主張 → 根拠fold` の順にする。マンダラはLLM文章や証拠文を再解釈せず、証拠層がLLM応答の受理後に組み立てる `visual.schemaVersion = 2` だけで描画する。内容は対象者全員の生成軸、NFKC完全一致語、全16 UAAMサブ指標のコホート内percentileである。UAAMの同値点・近接点はx座標を変えず縦方向へ積み、円の配置・距離は相性の強さや優劣を表さない。

本人が入力したTop5の語はLLMに送信されない。生成軸名は別名化プロフィールの一部としてLLMに渡る。完全一致語は受理済み本文へ表示専用データとして後置し、v2かつ一致語がある共有だけで、対象者間への表示同意とこの送信境界を説明する。表示名は分析入力から分離した編集可能ラベルで、LLMには送らず、レポート・共有ページ・先頭書記素の分布点にだけ使う。

v2のLLM出力には、チーム目的がある時だけ `unmetFunctionCandidate` を最大1件許可する。これは人物の不足ではなく目的達成に必要な未充足「機能」の仮説であり、`欠員` はvalidatorで拒否する。確認質問は最近の具体的な協働場面を対象に、仮説を支持する説明と中立な別要因を識別できる一問とする。保存済みv1共有は従来の本文契約で引き続き閲覧でき、新規分析・共有だけをv2とする。
...
分析実行時のレポート本文は保存しない。倫理監査として `compat_audits` に、実行者UID、モード、人数、内部/公開の別、同意確認、目的入力の有無、モデル、成功/失敗だけを書く。対象者ID、氏名、目的本文は保存しない。

### 期限付き共有URLを追加した理由（2026-07-13 owner decision）

当初のMVPは「保存レポートなし・公開結果URLなし」とした。対象外の人へ人物分析が流出する事故を、機能ごと無くすためである。その後ownerから、分析対象の4名本人が結果品質と実際のアプリ表示を確認できるよう、Telegramグループへ共有URLを送る明示要望があった。この限定目的に対し、次の条件をすべて満たすbearer URLとして方針を改めた。

- 分析同意とは別に、発行直前に対象者全員が結果共有にも同意したことを確認する。v2に完全一致語がある場合だけ、その語が表示されることと「本人が入力したTop5の語はLLMに送信されない。生成軸名は別名化プロフィールの一部としてLLMに渡る」境界も同じ同意欄で明示する。未確認ならUIとAPIの両方で発行しない
- URLはUUID v4で生成し、公開APIは単一doc取得だけとする。一覧・検索・存在理由の区別を設けない
- 保存前に、分析レスポンスの完全なフィールド集合、データ充足度、証拠ledger、claim契約、禁止表現、人数とaliasをサーバで再検証する。不正なら422で、docも監査も書かない
- 保存対象は検証済みレポート、表示用ラベル、mode、目的入力の有無、同意確認、発行者UID、作成・期限・失効状態だけとする。メール、対象者UID、生回答、元プロフィール、目的本文は保存しない
- アクセス期限は発行から30日、管理者は即時失効できる。不明・期限切れ・失効済みは同じ404にする
- 共有画面は「対象者本人への限定共有」「同意なしに再共有しない」「訂正・共有停止は発行者へ連絡」を明示し、人事評価・採用評価への非流用注記を共通rendererで保つ
- HTML routeと公開APIに `no-store`、`no-referrer`、`noindex` を設定する。検索除外は秘密保持の代替ではなく、UUIDを知る人は読めることを管理画面にも明示する

本番デプロイチェックリスト:

- [ ] `COMPAT_SHARE_ORIGIN` を本番の固定originに設定する（本番ではHost系ヘッダから発行URLを生成しない）
- [ ] Firestore TTL policyを `compat_shares.expiresAt` に設定する（APIの期限判定は即時アクセス遮断、TTL policyは期限後の物理削除を担う）

これは認証済みの「4名だけ」を技術的に識別する方式ではない。パスコードは同じTelegram経路でURLと一緒に漏れやすく、個別tokenは4名が同じグループで見る今回の運用では訂正・失効を複雑にする割に実効差が小さいため採用しなかった。対象外共有が必要になった時点、または再共有事故が観測された時点で、本人認証付き閲覧へ移行する。

`compat_audits` の共有監査は `share_issued | share_revoked`、shareId、実行者UID、時刻だけを保存する。shareIdとレポート本文を同じログへ出さず、監査docにも対象者ラベル・目的・本文を複製しない。本人から訂正または停止要請があれば発行者が即時失効し、内容を直す場合は再分析・再同意・新URL発行とする。
...
### チーム16×16力マップ・受講者さがし・実名表示（2026-07-24 owner decision）

ソース: つかさ /goal 指示（UAAM式16×16でチームの強弱可視化・受講者のおすすめ・M1でなく実名・小5でなく自然な日本語・図を多用）＋ plan-with-debate 2ラウンド（Codex sol/terra・Kimi・Claude独立×2、fact-check は `debate/plan-debate-20260724-compat-matrix/`）＋ sol 8観点コードレビュー。

- **uaamMatrix は visual の外**: 16軸生スコアは分析レスポンスのトップレベル `uaamMatrix` に置き、schemaVersion 2 の visual 契約には一切触れない。共有発行はクライアント除去＋サーバー clone 除去＋hasExactKeys 完全一致（余分キー=422 fail-closed）＋CompatReport の専用 prop 描画の4層で遮断する。版数バンプ案は「読取時再検証で既存共有URLが無音404になる」ため棄却した。**共有検証キー表への memberScores 追加は禁止**（strip 漏れの誤修正が生スコアを共有保存に流す反転事故になるため）。
- **セルの意味論は個人ペアゾーンの集約**: セル(i,j) は各メンバー本人の `getZone(その人の軸i, 軸j)` の最大ゾーン＋担い手数。軸ごとの最大値を別人から合成して getZone に渡す案は「誰も発動できないペアが最上位色になる」ため棄却（debate R2 で Codex/Kimi が同時指摘）。呼称は「チームの強さ」でなく「チームの中にある力の地図」とし、凡例に「高い人が1人いる＝いつでも発揮できる、ではない」を明記。担い手ゼロ軸は DORMANT と別の「データなし」。
- **受講者さがしの二段階開示**（順位付けの禁止は 2026-08-01 に「学び合い指標」に限って解除。上記「学び合い指標を解禁した理由」を参照）: 不足軸（チーム最高値<12 または データなし）を16以上で持つ内部受講者のみ対象。第1段階は軸ごとの人数集計だけ（氏名なし）、第2段階は個別同意確認チェック後に氏名＋該当軸チップを表示名照合順で表示。有用度ソート・件数バッジ・評価コピーは禁止。当初の「弱セル数降順Top5」は実質ランキングとして棄却。public メンバー混在は 400（架空URL による全受講者列挙の遮断）。profileId はレスポンスに含めない。実行は両段階とも compat_audits に記録。**2026-08-01 追加**: 選択人数の下限を1名へ拡張し（1人モード）、第2段階に学び合い指標つきの上位10件を追加した。1人モードは `compat-analyze` を呼ばず（2レンズは2名以上でしか成立しない）、本人の16×16マップと診断で見つかった軸を決定論だけで表示する。共有URL発行は1人モードでは行わない（共有契約の mode は `pair` / `team` のまま無変更）。
- **実名はクライアント表示のみ**: pair 別名を A/B→M1/M2 に統一し、M トークンだけを英数字境界（lookbehind 不使用）で表示名へ置換。LLM入出力・共有保存docは別名のまま。既知の限界: 実データ中に「M1」等の表記が来た場合の誤置換（構造化トークン化は将来の LLM 出力契約 v3 の第一候補）。
- **ゾーン判定の正本は `src/lib/uaamZones.js`**: AllPairsTriangle と CompatMatrix が共用。閾値変更禁止・21×21全域同値テストで固定。
