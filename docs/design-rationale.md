...
## 8.1. SelectScreen 直近診断結果インライン表示 (issue #47 派生)

### 採用案
`/api/me/diagnosis-status` の各 kind に **`recentAttempts: [{id, label, createdAt, isLegacy}]`** を最大 2 件、最新順で追加。SelectScreen の各カード CTA 直下にクリッカブル行として描画し、HistoryScreen と同じ `ATTEMPT_ID` 経路 (`/history/{kind}/{attemptId}`) で navigate する。

```
レスポンス shape (各 kind):
{
  committedCount, attemptCount, hasPending, pendingAttemptId,
  hasResult, isStartBlocked,
  recentAttempts: [
    { id: 'attempt-2', label: '問いに火を灯す人', createdAt: '2026-05-02T...', isLegacy: false },
    { id: 'attempt-1', label: '問いに火を灯す人', createdAt: '2026-04-30T...', isLegacy: false }
  ]
}
```

### なぜこうしたか
- 「診断済み (1/2)」バッジは右上 18px のスロットに小さく出るだけで、ユーザーが「やっていない」と誤認して再度開始する事故が観測されていた（issue #47 のスクショ state-count1）
- 最も近い場所＝CTA 直下に履歴サマリーを置けば「やった結果」の存在感が増し、再診断の暴発を防げる

### 棄却した代替案
- **(A) CTA ラベル動的切替（「もう一度診断する」）**: API 変更不要だが、結果テキストが見えないので根本解決にならない
- **(B) 新規エンドポイント `/api/me/select-screen-overview`**: 責務分離は綺麗だが、SelectScreen 用途は `diagnosis-status` の派生情報に過ぎず、エンドポイント増加のメンテコスト増の方が大きい
- **(C) SelectScreen から `/api/me/history?kind=X` を 2 fetch**: ログイン直後の fetch が 1→3 に増えてレイテンシ体感悪化、`/api/me/history` は full attempt + integrations を返すため帯域も大きい

Round 1 debate で Gemini が「ステータスAPIにエンティティリストを混ぜるのは SRP 違反」と批判したが、`pendingAttemptId` 等で既に純粋 count API ではなく派生情報が含まれている。fetch 1 回で SelectScreen が完成する利点を取って (A) を採用。確定: 2026-05-05 つかさ判断。

### 共存ケースの取り扱い
`shared/attemptLogic.js` の `listFromAttempts` と挙動を揃え、**attempts コレクションに 1 件以上 doc がある場合は legacy fallback を合成しない**（最大 2 件は subcollection からのみ）。attempts が空のときだけ `legacyDocToAttempt(parentData, kind)` を 1 件として返す（id: `'legacy-fallback'`, isLegacy: true）。

2026-05-05 監査で本番 Firestore に bare legacy parent doc が saikaku 52名 / uaam 1名残存することを確認（`scripts/_audit-legacy-parents.mjs`）。これらのユーザーには legacy fallback 経路で 1 行表示される。legacy 削除は別 issue 化。

### Firestore 複合インデックス追加
`recentAttempts` 取得は `parentRef.collection('attempts').where('status', '==', 'committed').orderBy('createdAt', 'desc').limit(2).select('summary', 'full', 'createdAt', 'isLegacy')` を新規追加。本番デプロイ時に **`firestore.indexes.json` に `attempts(status ASC, createdAt DESC)` の複合インデックスを追加し `firebase deploy --only firestore:indexes` が必要**。エミュレータは index 不要で動くので CI/local は通るが、本番反映前に必ずインデックスを deploy する。`select()` で `raw.input.answers` 等の重量 field を除外し、UAAM では特に帯域を削減。

### Cache shape 防御
`useDiagnosisStatus.js` の `isValidStatusShape` は `recentAttempts` を要求しないため、旧 shape の sessionStorage cache（`recentAttempts` なし）が pass する。SelectScreen 側は **`status?.[kind]?.recentAttempts ?? []`** で防御し、`undefined.map` で crash しない（`tests/unit/ui/select-recent-cache-shape.test.jsx` で実証）。

### HistoryScreen との共通化
`getAttemptOrdinal` / `getAttemptLabel` / `formatAttemptDate` / `getAttemptDate` を `src/utils/attemptDisplay.js` に切り出し、HistoryScreen / SelectScreen の両方から import。`getAttemptLabel` は新 shape (`attempt.label`) を最優先、旧 shape (`attempt.summary?.kakuchiiki` / `attempt.summary?.typeName`) を fallback として支える。これにより両画面で同じ表記揺れ（「1回目」/「保存済み履歴」、`Intl.DateTimeFormat('ja-JP')` 形式）が保証される。

### overlay button との z-index 設計
セクション 8 の「透過オーバーレイ Block Link」パターンとの両立: recentAttempts 行は `position: relative; zIndex: 2` で overlay (z-index 1) より前面に置く + `onClick` 内で `event.stopPropagation()` を発火し、行クリックがカード全体クリック (= 診断開始) に propagate しないようにする。Enter/Space キー操作でも同じ防御を行う。

### 観測結果
- vitest unit: 68/68 pass (新規 cache-shape テスト 1件 + 既存テスト全て pass)
- test:api: 15/15 pass (既存 9 件 + 新規 5 ケース: 0件 / 1件 committed / 2件 committed / legacy / pending+committed)
- test:e2e (Playwright): 31/31 pass (新規 4 シナリオ: count=1 / count=2 古い行クリック / legacy / UAAM count=1)
- screenshots/issue-47/ の 28 枚を recentAttempts 行を含む新 baseline で更新
