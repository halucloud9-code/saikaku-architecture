...
## 15. 管理画面 統合分析タブのマウント時プリフェッチ化 (2026-05-12)

### 背景
Section 12 で確定した「タブ初回表示時にのみ fetch」(`遅延取得`) により、`/admin` 初期表示時の統合分析バッジは `'統合分析（—）'` のまま、タブをクリックするまで件数が出ない状態だった。才覚領域・UAAM 診断のバッジは初回フェッチ完了で数字に確定するため、統合分析だけ「件数不明」期間が永続化し、UX の不揃いが管理者から指摘された（"統合分析がすぐに出ない"）。

### 採用案
`AdminScreen` マウント時の並列フェッチ群（`fetchUsers` / `fetchSummaryCounts`）に `fetchIntegrations` を追加。Section 12 の「遅延取得」方針を撤回し、他タブと同じ初回フェッチ完了タイミングでバッジが数字に確定する仕様へ揃える。

実装の要点:
- `integrationsLoading` 初期値を `useState(true)` に変更（マウント直後から「読み込み中」状態）
- `integrationsReqIdRef` を新設し、`summaryCountsReqIdRef` と同じ世代番号パターンで stale-response 破棄
- タブ起点 lazy effect は **保険として残す**（初回フェッチ失敗 → ユーザーがタブを開いて再試行）が、`integrationsLoading` チェックを guard に加えて in-flight 中の二重発火を防ぐ
- 「統合分析は未読込です」プレースホルダ分岐は到達不可になるため削除、未読込状態は loading spinner に統合
- ユーザー削除ハンドラ（uid / email 経路）で `integrations` state を整合性のため更新

### なぜこうしたか

**プリフェッチ化を採用した理由**:
- バッジ件数の挙動を 3 タブで統一することで「読み込み完了 = 全タブ確定」という管理者の予測モデルに揃う
- 統合分析データは admin にとって主要観測対象。lazy load は「タブを開かないと件数が分からない」ストレスを生む

**集計エンドポイント分離（代替案）を棄却した理由**:
- `/api/admin/summary-counts` に `breakdown.integrations.total` を aggregate count() で追加する案は debate で検討した
- `collectionGroup('integrations')` の `parent.parent.parent.id !== 'uaam_results'` 名前空間ガード + legacy fallback の重複除外を aggregate count() で厳密に再現できず、バッジ件数と実 `integrations.length` の乖離リスクがある
- 「数字が出るが実数と違う」UX は混乱を招くため棄却

**API 軽量化（代替案）を棄却した理由**:
- `/api/admin/integrations` の `select()` フィールド絞り込みや `where('integrationUpdatedAt', '>=', ...)` での絞り込みは本質的な解決だが、PR スコープが拡大して回帰リスクが上がる
- 本 PR は fetch タイミング修正に限定し、API 軽量化は **別 issue として切り出す**（doc 数 1000 超で再評価）

**lazy effect を残した理由**:
- マウント時 prefetch が失敗した場合の保険として、タブをクリックすれば再試行できるよう保持
- `integrationsLoading` ガードで in-flight 中の二重発火を防ぎつつ、依存配列は `[tab, integrationsLoaded]` のまま（`integrationsLoading` を依存に入れると失敗時に自動再試行ループになる）。明示的なリカバリは `integrationsError` 表示時の「読み直す」ボタンに任せる

**削除ハンドラ整合性**:
- uid 経路は `setIntegrations(prev => prev.filter(it => it.uid !== uid))` で対応
- email 経路は integration item に email が含まれるとは限らないため `fetchIntegrations()` 再フェッチで整合性確保

### スコープ
- 編集: `src/screens/AdminScreen.jsx`（fetch タイミング・state guard・削除ハンドラ）
- 編集: `src/screens/__tests__/AdminScreen.integrations-tab.test.jsx`（lazy 前提 → prefetch 前提に書き換え）
- 新規: `docs/admin-screen.md`（運用メモ・観測ガイド）
- 非変更: `/api/admin/integrations` レスポンス形・パス・実装、他タブ、`integrationsError` 振る舞い

### 観測
- vitest unit: 11 files / 70 tests pass
- E2E (admin-integrations 3 specs): all passed
- 本番デプロイ後の観測指標は `docs/admin-screen.md` 参照

### 参照
- 実装: `src/screens/AdminScreen.jsx`
- プラン: `plans/admin-integrations-tab-prefetch.md`
- Codex 委任応答: `codex/impl-1-output/`
- Codex code-review: `codex/codex-review-20260512/`
- 関連 Section: §12 (統合分析タブ初出)
