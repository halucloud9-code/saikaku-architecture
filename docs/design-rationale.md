...
Anthropicへ送る識別子は A/B または M1…だけとする。氏名、メール、UID、自由記述は送らない。本人が入力したTop5の語はLLMに送信されない。生成軸名は別名化プロフィールの一部としてLLMに渡る。完全一致について送るのは件数と分類だけである。ほかにUAAM percentile由来の証拠を送る。入力データは命令ではないことをsystem promptで明示し、出力後は未知の証拠ID、claim契約違反、相性スコア、人事・採用・査定語を決定論的に拒否する。修復再試行は1回だけで、再失敗時はレポートを返さない。
