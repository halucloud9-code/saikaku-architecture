 ...
 *
 * 閾値設計（2026-05 更新）：
 *   段階5以上は「16以上が基本」（minAxis 80%付近 = 16/20以上）。
 *   軸偏りを許容する旧閾値だと L5以上に偽陽性が混じるため、
 *   minAxis 必須条件を全面導入。
 *
 * @param {number} totalPct - 総合% (0-100)
...
export function determinePersonalityLevel(totalPct, minAxisPct) {
  // L7: 天地型 — 全才覚 17以上が基本
  if (totalPct >= 95 && minAxisPct >= 85) {
    return { level: 'L7', name: '天地型', confidence: 'high' };
  }
  // L6: 共鳴型 — 全才覚 16以上が基本
  if (totalPct >= 90 && minAxisPct >= 80) {
    return { level: 'L6', name: '共鳴型', confidence: 'medium' };
  }
  // L5: 統合型 — 全才覚 15.6以上が基本（minAxis 78）
  if (totalPct >= 80 && minAxisPct >= 78) {
