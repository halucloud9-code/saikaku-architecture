...
export function checkValidity(vAnswers, mainAnswers) {
  ...
  if (level >= 6) {
    flags.push({ level: 'critical', type: 'inflation_strong', message: '客観視の精度に課題', detail: 'V1・V2・V3すべて最低評価。コーチング介入を強く推奨します。' });
  } else if (level >= 5) {
    flags.push({ level: 'critical', type: 'inflation_strong', message: '自己評価インフレ傾向強', detail: `合計${totalPts}pt。結果の解釈に補正が必要な可能性があります。` });
  } else if (level >= 4) {
    flags.push({ level: 'warning', type: 'inflation', message: '自己評価に歪みの可能性', detail: `合計${totalPts}pt。客観視をコーチングテーマとして扱うことを推奨します。` });
  } else if (level >= 2) {
    flags.push({ level: 'info', type: 'mild', message: 'わずかな傾向あり', detail: `合計${totalPts}pt。参考程度に留意してください。` });
  }

  return { hasFlags: flags.length > 0, flags, vFlags, totalPts, level };
}
