    ...
    >
      ...
      >
        ...
        {(() => {
          ...
          const flagEmoji = (id) => vf[id] === 'critical' ? '🏴🏴' : vf[id] === 'warning' ? '🏴' : null;
          ...
          return (
            <div style={{ marginBottom: 20, background: lvBgs[level], border: `1px solid ${lvColors[level]}40`, borderRadius: 8, padding: '10px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <span style={{ marginLeft: 'auto', background: lvColors[level], color: '#fff', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>
                  Lv.{level}
                </span>
              </div>
        ...
        {u.scores && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#7A7060', letterSpacing: '0.1em', marginBottom: 12 }}>才覚発動領域</div>
            <ActivationMatrix scores={u.scores} maxSub={20} />
          </div>
        )}
...
const GRADE_COEFF = {
  ...
  'C+': 0.75, 'C': 0.70, 'C-': 0.65, 'D+': 0.63, 'D': 0.60,
  'E+': 0.55, 'E': 0.50,
};
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 12px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                        <span style={{ fontSize: 11, letterSpacing: '0.05em', color: isCritical ? '#A84432' : '#7A7060' }}>
                          {['V1','V2','V3'].map(id => (
                            <span key={id} style={{ marginRight: 3 }}>{id}{flagStr(id)}</span>
                          ))}
                        </span>
                      </td>
                      {/* Lv */}
