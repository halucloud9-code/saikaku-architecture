...
export default function InputScreen({ user, error, isAdmin, onSubmit, onAdmin, onLogout, onBack }) {
  ...
  const CATEGORIES = [
    {
      ...
      labelTop5: <>最も大切にしている<span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 24, fontWeight: 900, color: '#FFD700' }}>5</span>つ</>, top5: valueTop5, setTop5: setValueTop5,
    ...
    },
    {
      ...
      labelTop5: <>最も得意な<span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 24, fontWeight: 900, color: '#FFD700' }}>5</span>つ</>, top5: talentTop5, setTop5: setTalentTop5,
    ...
    },
    {
      ...
      labelTop5: <>最も夢中になる<span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 24, fontWeight: 900, color: '#FFD700' }}>5</span>つ</>, top5: passionTop5, setTop5: setPassionTop5,
  ...
  const QUESTIONS = [
    ...
    { num: 'Q3', q: <>才覚領域を全力で生き続けた<span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 18, fontWeight: 900, color: '#FFD700' }}>10</span>年後<br />あなたの周りはどんな影響や変化を遂げていますか？</>, val: q3, set: setQ3, ph: '人・組織・社会・世界...どんな変化が起きているか' },
  ];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          Unique Ability <span style={{ color: '#FFD700', fontSize: 12, fontWeight: 400 }}>Architecture</span>
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {isAdmin && (
            <button onClick={onAdmin} style={{ fontSize: 12, color: '#FFFFFF', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>管理</button>
          )}
          {user.photoURL && (
          ...
          <button onClick={async () => { await signOutUser(); onLogout(); }} style={{
            fontSize: 12, color: '#FFFFFF', background: 'none', border: 'none', cursor: 'pointer', padding: 0,
          }}>ログアウト</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <div style={{
          ...
          {/* コーナー装飾 */}
          <div style={{ position: 'absolute', top: -1, left: 20, width: 40, height: 3, background: '#FFD700', borderRadius: '0 0 2px 2px' }} />
          <div style={{ position: 'absolute', top: -1, right: 20, width: 40, height: 3, background: '#FFD700', borderRadius: '0 0 2px 2px' }} />
          <div style={{ position: 'absolute', bottom: -1, left: '50%', transform: 'translateX(-50%)', width: 60, height: 3, background: 'linear-gradient(90deg, transparent, #FFD700, transparent)', borderRadius: '2px 2px 0 0' }} />

          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <div style={{ width: 50, height: 1, background: 'linear-gradient(90deg, transparent, #FFD700)' }} />
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.3em', color: '#FFD700', textTransform: 'uppercase' }}>Discovery</span>
            <div style={{ width: 50, height: 1, background: 'linear-gradient(90deg, #FFD700, transparent)' }} />
          </div>
          <h2 style={{
            fontFamily: "'Noto Serif JP', 'Times New Roman', serif",
          ...
          <p style={{ fontSize: 13, color: '#FFFFFF', margin: 0, lineHeight: 1.8 }}>
            <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 26, fontWeight: 900, color: '#FFD700' }}>3</span>つの問いに答えるだけで、あなただけの才覚領域が明らかになります
          </p>
        </div>
        ...
        <div style={{
          ...
          {CATEGORIES.map((cat, ci) => (
            <div key={cat.key} style={{ marginBottom: ci < 2 ? 36 : 0 }}>
              <div style={{
                ...
                padding: '24px 22px',
                marginBottom: 16,
                boxShadow: `0 4px 20px rgba(0,0,0,0.2), 0 0 0 1px ${cat.color}15`,
              }}>
                ...
                <p style={{ fontSize: 13, color: '#FFFFFF', margin: '0 0 20px', lineHeight: 1.6 }}>{cat.sublabel}</p>
                ...
                <div style={{ marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, color: '#FFFFFF', fontWeight: 600 }}>{cat.labelOthers}</span>
                    <span style={{ fontSize: 11, color: '#FFFFFF' }}>（任意）</span>
                  </div>
                ...
                <p style={{ fontSize: 12, color: '#FFFFFF', margin: '8px 0 0', lineHeight: 1.6 }}>
                  {cat.hint}
                </p>
              </div>
            </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#FFFFFF', letterSpacing: '0.1em' }}>DEEP QUESTIONS</span>
            </div>
          ...
          {displayError && (
            <div style={{
              ...
              marginTop: 24, fontSize: 13, color: '#FF6B6B',
            }}>{displayError}</div>
          )}
          ...
          <p style={{ textAlign: 'center', fontSize: 12, color: '#FFFFFF', marginTop: 14 }}>
            ※ 解析には約<span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 15, fontWeight: 800 }}>30</span>秒かかります
          </p>
