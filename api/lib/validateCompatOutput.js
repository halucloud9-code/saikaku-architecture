...
export function validateCompatOutput(raw, evidenceLedger) {
  const errors = [];
  if (!exactKeys(raw, ['dataSufficiency', 'lenses'])) {
    return { ok: false, errors: ['root: dataSufficiency と lenses だけが必要です'] };
  }
